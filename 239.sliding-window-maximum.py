...
class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        while right < nums_length:
            while q and nums[q[-1]] < nums[right]: # if the new incoming value is greater than the last value in deque
                q.pop()
            
            q.append(right) # always append right side index in queue, on the left side of queue, the index correspond to larger value, while right side has smaller value
            
            if q[0] < left: # max value index out of range
                q.popleft()
            
            if right + 1 >= k: # window is formed, need to adjust left pointer & append max value to res
                res.append(nums[q[0]])
                left += 1
